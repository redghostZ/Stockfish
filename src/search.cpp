...
namespace {
  ...
  int stat_bonus(Depth d) {
    return d > 15 ? -8 : 19 * d * d + 154 * d - 132;
  }
...
void Search::init() {
  ...
  for (int i = 1; i < MAX_MOVES; ++i)
      Reductions[i] = int((24.7 + std::log(Threads.size())) * std::log(i));
}
         ...
         && !(Limits.depth && mainThread && rootDepth > Limits.depth))
  {
      ...
      for (pvIdx = 0; pvIdx < multiPV && !Threads.stop; ++pvIdx)
      {
          ...
          if (rootDepth >= 4)
          {
              ...
              int dct = ct + (101 - ct / 2) * prev / (abs(prev) + 155);
      ...
      {
          double fallingEval = (331 +  6 * (mainThread->bestPreviousScore - bestValue)
                                    +  6 * (mainThread->iterValue[iterIdx]  - bestValue)) / 704.0;
          fallingEval = Utility::clamp(fallingEval, 0.5, 1.5);

          // If the bestMove is stable over several iterations, reduce time accordingly
          timeReduction = lastBestMoveDepth + 9 < completedDepth ? 1.93 : 0.91;
          double reduction = (1.42 + mainThread->previousTimeReduction) / (2.27 * timeReduction);
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        Value raisedBeta = beta + 187 - 45 * improving;
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
          ...
          else
          {
              ...
              if (   !givesCheck
                  && lmrDepth < 6
                  && !ss->inCheck
                  && ss->staticEval + 270 + 383 * lmrDepth + PieceValue[MG][type_of(pos.piece_on(to_sq(move)))] <= alpha)
                  continue;
